	...
	public partial class _default : System.Web.UI.Page
	{
			...
			foreach (var channel in context.Channels)
			{
				...
				if (channel.State == ChannelState.Running)
				{
					...
					previewPlayerURI.Text = BuildPlayerHTML(PreviewURL, 280, 500,
						@" data-setup='{""disableUrlRewriter"": true}'");
					...
					if (channel.Programs.Count() > 0)
					{
						...
						if (url != null) {
							...
							var publishPlayerURLCell = new TableCell();
							var publishPlayerURLCellText = new TextBox();
							publishPlayerURLCellText.Text = originURL;
							publishPlayerURLCell.Controls.Add(publishPlayerURLCellText);
							...
							publishPlayerCell.Controls.Add(publishPlayerURLCell);
					...
					string SourceOption)
		{
			return new StringBuilder(1024)
				.Append(@"<video id=""videoplayer")
				.Append(ampCount ++)
				.Append(@""" class=""azuremediaplayer amp-default-skin"" autoplay controls width=""")
				.Append(weight)
				.Append(@""" height=""")
				.Append(height)
